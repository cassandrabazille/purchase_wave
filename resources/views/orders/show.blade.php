    ...
    <div class="container">
        ...
        <div class="padding-top-2">
            ...
            <div>
                @if(session('success'))
                    <div
                        class="light-green-background black-color margin-top-2 border-radius-1 padding-3 font-size-1-4 text-align">
                        {{ session('success') }}
                    </div>
                @endif
            ...
            <div class="border-radius-1 black-box-shadow padding-3">
                <div>
                    ...
                    <p class="padding-bottom-2">
                        La commande <strong>{{ $order->reference }}</strong>
                        fournie par <strong>{{ $order->supplier->name }}</strong> est <strong>{{ $order->status }}</strong>.
                        ...
                        <table class="width-100pct font-size-1-6 border-collapse no-border">
                            <thead>
                                <tr class="grey-background">
                                    <th class="font-size-1-4 font-weight-bold text-align-left padding-1-6-2">Ref. produit
                                    </th>
                                    <th class="hide-mobile font-size-1-4 font-weight-bold text-align-left padding-1-6-2">
                                        Prix unitaire (en €)</th>
                                    <th class="hide-mobile font-size-1-4 font-weight-bold text-align-left padding-1-6-2">
                                        Quantité</th>
                                    <th class="font-size-1-4 font-weight-bold text-align-left padding-1-6-2">Total de la
                                        ligne</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-align-left padding-1-6-2 with-bottom-border">
                                            <div class="align-items-center gap-1-5">
                                                <form action="{{ route('products.show', $orderitem->product->id) }}"
