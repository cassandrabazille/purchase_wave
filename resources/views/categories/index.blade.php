...
@endif
     ...
     <button class="responsive-button black-background white-color font-poppins-ss font-size-1-4 align-items-center justify-center width-27 height-4-4 border-radius-1 black-box-shadow no-border cursor-pointer">Créer une nouvelle catégorie</button>
      </a>
    </div>

      <h2 class="padding-2">Catégories</h2>
      <h3 class="medium-grey-color font-weight-500 padding-left-2 padding-bottom-2">Liste de toutes les catégories enregistrées</h3>
      <div>
      ...
      <td class="text-align-left padding-1-6-2 with-bottom-border">
        ...
        <img src="{{ asset('images/view.png') }}" alt="Icône oeil pour voir le détail de la catégorie" class="eye-icon" />
        </button>
        </form>
           <form action="{{ route('categories.edit', $category->id) }}" method="GET" style="display: inline;">
        <button type="submit" style=" background: none; border: none" class="cursor-pointer">
        <img src="{{ asset('images/edit.png') }}" alt="Icône crayon pour modifier la catégorie" class="pencil-icon"/>
        </button>
        </form>
        ...
        <button type="submit" class="cursor-pointer no-border no-background"><img
          src="{{ asset('images/delete.png') }}" alt="Icône croix pour supprimer la catégorie" class="delete-icon width-2 height-2" />
        </button>
        </form>
        </div>
      </td>
