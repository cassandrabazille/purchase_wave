...
@endif
                ...
                <select name ="items[0][product_id]" class="text-align-center margin-bottom-2 black-background white-color border-radius-0-6 width-11 height-3 no-border" required>
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}">
                            {{ $product->reference }} {{ $product->slug }} {{ number_format($product->price, 2, ',', ' ') }} 
                        </option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="justify-center">
        <button type="submit" class="responsive-button justify-center align-items-center blue-background hover-blue font-poppins-ss font-size-1-4 white-color normal-font-weight width-15 height-5 margin-top-1 border-radius-3-4 no-border cursor-pointer">Enregistrer</button>
    </div>
</form>
</div>
            </div>
